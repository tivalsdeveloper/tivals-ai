...
self.addEventListener('fetch', event => {
  ...
  if (event.request.mode === 'navigate') {
    event.respondWith(fetch(event.request,{cache:'no-store'}).then(async response => {
      ...
      enhanced = enhanced.replace('</head>', '<link rel="stylesheet" href="./tivals-theme.css?v=17"></head>');
      enhanced = enhanced.replace('</body>', '<script src="./mobile-nav.js?v=17"></script></body>');
