...
(()=>{
 ...
 /* Providers such as Qwen may emit XML-ish tool calls instead of special tokens. Remove the complete protocol block, not just the tags. */
 ...
 function clean(v){if(typeof v!=='string')return v;let s=v.replace(TOOL_BLOCK_RE,'').replace(XML_TOOL_BLOCK_RE,'').replace(XML_INVOKE_BLOCK_RE,'').replace(XML_FUNCTION_BLOCK_RE,'').replace(TOKEN_RE,'').replace(TOOL_LINE_RE,'').replace(SAFETY_RE,'').replace(XML_ORPHAN_RE,'');s=s.replace(/(?:\n\s*){3,}/g,'\n\n').trim();return s}
