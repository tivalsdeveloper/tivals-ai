...
(()=>{
 ...
 function cleanHistory(){try{const raw=localStorage.getItem(HIST);if(!raw)return;const a=JSON.parse(raw);if(!Array.isArray(a))return;let changed=false;for(const m of a){if(m&&m.role==='assistant'&&typeof m.content==='string'&&!m.content.startsWith('[YouTube Search]')){const c=clean(m.content).trim();if(c!==m.content){m.content=c;changed=true}}}if(changed)localStorage.setItem(HIST,JSON.stringify(a))}catch(e){console.warn('Tivals sanitizer history cleanup failed',e)}}
