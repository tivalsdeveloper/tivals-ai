...
(() => {
  ...
  function imageCard(item,prompt){const msg=add('ai',''),bubble=msg.querySelector('.bubble');fillImageCard(bubble,item.url,prompt);scrollToLatest()}
  function restoreSavedImages(root=document){
    const bubbles=root.querySelectorAll?.('.msg.ai .bubble:not([data-tivals-image]), .ai .bubble:not([data-tivals-image])')||[];
    bubbles.forEach(bubble=>{const saved=parseSavedImage(bubble);if(saved)fillImageCard(bubble,saved.url,saved.prompt)});
  }
  async function runImage(text){if(!window.TivalsImageGenerator){add('ai','Image generator is still loading. Please try again.');return}const p=cleanPrompt(text);add('user',text);history.push({role:'user',content:text});save();const wait=add('ai','Generating your image…');busy(true);try{const item=await window.TivalsImageGenerator.generate(p);wait.remove();imageCard(item,p);history.push({role:'assistant',content:'[Generated image] '+p+'\n'+item.url});save()}catch(error){wait.querySelector('.bubble').innerHTML='<p>'+esc(error.message||'Image generation failed. Please try again.')+'</p>'}finally{busy(false)}}
  function intercept(){const text=promptBox.value.trim();if(!text||!isImageRequest(text))return false;promptBox.value='';promptBox.style.height='auto';runImage(text);return true}
  sendBtn.addEventListener('click',e=>{if(intercept()){e.preventDefault();e.stopImmediatePropagation()}},true);promptBox.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing&&isImageRequest(promptBox.value)){e.preventDefault();e.stopImmediatePropagation();intercept()}},true);
  const tools=document.querySelector('.tools');if(tools&&!document.querySelector('#tivalsGenerateImageTool')){const b=document.createElement('button');b.className='toolBtn';b.id='tivalsGenerateImageTool';b.type='button';b.textContent='▧ Generate image';b.onclick=()=>{promptBox.value='Generate an image of ';promptBox.focus();promptBox.setSelectionRange(promptBox.value.length,promptBox.value.length)};tools.prepend(b)}
  const sideTools=document.querySelector('.sideTools');if(sideTools&&!document.querySelector('#sideImageGenerator')){const b=document.createElement('button');b.className='sideTool';b.type='button';b.id='sideImageGenerator';b.textContent='Image generator';b.onclick=()=>{promptBox.value='Generate an image of ';promptBox.focus();document.querySelector('#sidebar')?.classList.remove('open');document.querySelector('#sideOverlay')?.classList.remove('open')};sideTools.prepend(b)}
