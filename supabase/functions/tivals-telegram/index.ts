...
function splitText(text: string, limit = 3500) {
  ...
  const out: string[] = []; let r = text;
  while (r.length > limit) {
    let c = r.lastIndexOf("\n\n", limit); if (c < 1800) c = r.lastIndexOf("\n", limit); if (c < 1800) c = r.lastIndexOf(" ", limit); if (c < 1800) c = limit;
    out.push(r.slice(0,c).trim()); r = r.slice(c).trim();
  }
...
async function connectMenu(chatId: number|string, tg: number, business?: string) {
  ...
  for (const [provider,label] of [["gmail","📧 Connect Gmail"],["github","🐙 Connect GitHub"]] as const) {
    try { const d = await oauthCall("create_link", tg, provider); if (d?.url) rows.push([{ text: label, url: d.url }]); } catch {}
  }
  if (!rows.length) return sendFormatted(chatId, "⚠️ Account connections are temporarily unavailable.", business);
  const p: any = { chat_id: chatId, text: "🔐 <b>Connect accounts to Tivals AI</b>\n\nEach connection is private to your Telegram account. You can disconnect it at any time.", parse_mode: "HTML", reply_markup: { inline_keyboard: rows } };
  if (business) p.business_connection_id = business;
  await telegram("sendMessage", p);
}
async function accountStatus(chatId: number|string, tg: number, business?: string) {
  const d = await oauthCall("status", tg); const list = Array.isArray(d?.connections) ? d.connections : [];
...
function gmailIntent(text: string): { matched: boolean; query: string; title: string } {
  ...
  if (/\b(?:in|from)\s+my\s+(?:gmail|email|inbox)\b/i.test(t) && /\b(?:find|search|check|show|read)\b/i.test(t)) return { matched: true, query: t.replace(/\b(?:find|search|check|show|read)\b/ig, "").replace(/\b(?:in|from)\s+my\s+(?:gmail|email|inbox)\b/ig, "").trim(), title: "Email search" };
...
async function searchYouTube(query: string) {
  ...
  const d = await r.json().catch(() => ({})); if (!r.ok) throw new Error(d?.details || d?.error || "YouTube search failed."); return Array.isArray(d?.videos) ? d.videos : [];
}
...
function imagePrompt(text: string) {
  for (const p of [/^\s*(?:create|generate|make|draw)\s+(?:me\s+)?(?:an?\s+)?(?:image|picture|photo|artwork|illustration)\s+(?:of\s+)?(.+?)\s*[.!]?\s*$/i,/^\s*(?:create|generate|make|draw)\s+(.+?)\s+(?:image|picture|photo|artwork|illustration)\s*[.!]?\s*$/i,/^\s*image\s+(?:of\s+)?(.+?)\s*[.!]?\s*$/i]) { const m = text.match(p); if (m?.[1]) return m[1].trim(); }
  return "";
}
function findImageUrl(value: any): string {
  const seen = new Set<any>();
  const visit = (v:any):string => { if (v == null) return ""; if (typeof v === "string") return /^https?:\/\//i.test(v) && !/\.mp4(?:\?|$)/i.test(v) ? v : ""; if (typeof v !== "object" || seen.has(v)) return ""; seen.add(v); for (const k of ["url","image","imageUrl","image_url","output","media","data","images","result","results"]) if (k in v) { const f = visit(v[k]); if (f) return f; } for (const x of Array.isArray(v) ? v : Object.values(v)) { const f = visit(x); if (f) return f; } return ""; };
  return visit(value);
}
async function generateImage(prompt: string) {
  const key = Deno.env.get("SUPABASE_SERVICE_ROLE_KEY") || "";
  const r = await fetch(PIXAZO_STUDIO_URL, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${key}`, apikey: key }, body: JSON.stringify({ type: "image", prompt }) });
  const d = await r.json().catch(() => ({})); if (!r.ok) throw new Error(d?.error || "Image generation failed."); const u = findImageUrl(d); if (!u) throw new Error("Image provider returned no image URL."); return u;
}
async function sendPhoto(chatId:number|string, url:string, prompt:string, business?:string) {
  const p:any = { chat_id: chatId, photo: url, caption: `🎨 <b>Generated image</b>\n${esc(prompt.slice(0,700))}`, parse_mode: "HTML" }; if (business) p.business_connection_id = business; await telegram("sendPhoto", p);
}
function bytesToB64(bytes: Uint8Array) { let s=""; for(let i=0;i<bytes.length;i+=0x8000) s += String.fromCharCode(...bytes.subarray(i,Math.min(i+0x8000,bytes.length))); return btoa(s); }
async function telegramImageDataUrl(fileId:string) {
  const token = Deno.env.get("TELEGRAM_BOT_TOKEN") || ""; const info = await telegram("getFile", { file_id: fileId }); const path = info?.result?.file_path; if (!path) throw new Error("Telegram did not return the image path.");
  const r = await fetch(`${TELEGRAM_API}/file/bot${token}/${path}`); if (!r.ok) throw new Error("Could not download the Telegram image."); const bytes = new Uint8Array(await r.arrayBuffer()); if (bytes.length > 8*1024*1024) throw new Error("Image is too large."); const type = r.headers.get("content-type") || "image/jpeg"; return `data:${type};base64,${bytesToB64(bytes)}`;
}
function visionText(d:any) { const c=d?.choices?.[0]?.message?.content; if(typeof c==="string") return c.trim(); if(Array.isArray(c)) return c.map((x:any)=>typeof x==="string"?x:x?.text||"").join("").trim(); return ""; }
async function analyzeImage(dataUrl:string, question:string) {
  const open = Deno.env.get("OPENROUTER_API_KEY") || "";
  if (open) {
    try { const r=await fetch(`${OPENROUTER_BASE}/chat/completions`,{method:"POST",headers:{Authorization:`Bearer ${open}`,"Content-Type":"application/json","HTTP-Referer":"https://ai.tivalsdeveloper.site/","X-OpenRouter-Title":"Tivals AI"},body:JSON.stringify({model:"openrouter/free",messages:[{role:"user",content:[{type:"text",text:question||"Describe this image and explain important visible text or details."},{type:"image_url",image_url:{url:dataUrl}}]}],max_tokens:1400,temperature:0.2})}); const d=await r.json().catch(()=>({})); const txt=visionText(d); if(r.ok&&txt) return txt; } catch {}
  }
  const app=Deno.env.get("APPMIX_API_KEY")||""; if(!app) throw new Error("Vision is temporarily unavailable.");
  for(const model of ["openai/gpt-4.1-free","google/gemini-3-flash-preview-free"]) { try { const r=await fetch(`${APPMIX_BASE}/chat/completions`,{method:"POST",headers:{Authorization:`Bearer ${app}`,"Content-Type":"application/json"},body:JSON.stringify({model,messages:[{role:"user",content:[{type:"text",text:question||"Describe this image and explain important visible text or details."},{type:"image_url",image_url:{url:dataUrl}}]}],max_tokens:1200,temperature:0.2})}); const d=await r.json().catch(()=>({})); const txt=visionText(d); if(r.ok&&txt) return txt; } catch {} }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    if (photos.length || doc) {
      const fileId = doc?.file_id || photos[photos.length-1]?.file_id; await telegram("sendChatAction", { chat_id: chatId, action: "typing", ...(business ? { business_connection_id: business } : {}) }).catch(()=>{});
      const reply = await analyzeImage(await telegramImageDataUrl(fileId), caption); await sendFormatted(chatId, reply, business); return json({ ok:true, route:"vision" });
    }
    if (!text) return json({ ok:true, ignored:true });
    if (text === "/start" || text.startsWith("/start ")) { await sendFormatted(chatId, "👋 **Hi! I'm Tivals AI.**\n\nAsk questions, check Gmail, search YouTube, generate images, analyze photos, or connect Gmail and GitHub with /connect.", business); return json({ok:true}); }
    if (text === "/help") { await sendFormatted(chatId, "**Tivals AI**\n\n/connect — Connect Gmail or GitHub\n/accounts — Show connected accounts\n/emails — Show latest Gmail messages\n/unread — Show unread Gmail messages\n/disconnect_gmail — Disconnect Gmail\n/disconnect_github — Disconnect GitHub\n\nYou can also say “check my emails”, “emails from SPU”, or “search my emails for application”.", business); return json({ok:true}); }
    if (text === "/connect") { if (!tg) throw new Error("Telegram user ID is unavailable."); await connectMenu(chatId,tg,business); return json({ok:true,route:"connect"}); }
    if (text === "/accounts") { if (!tg) throw new Error("Telegram user ID is unavailable."); await accountStatus(chatId,tg,business); return json({ok:true,route:"accounts"}); }
    if (text === "/disconnect_gmail" || text === "/disconnect_github") { if (!tg) throw new Error("Telegram user ID is unavailable."); const provider = text.endsWith("gmail") ? "gmail" : "github"; await oauthCall("disconnect",tg,provider); await sendFormatted(chatId,`✅ ${provider === "gmail" ? "Gmail" : "GitHub"} disconnected.`,business); return json({ok:true,route:"disconnect"}); }
