(() => {
  ...
  fetch(WIDGET_API+'&models=1').then(response=>response.json()).then(data=>{const ids=(data.models||[]).map(item=>item.id).filter(Boolean);availableModels=[config.model,...ids].filter((id,index,all)=>all.indexOf(id)===index)}).catch(()=>{});
